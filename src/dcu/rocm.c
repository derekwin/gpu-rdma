...
static void initOnceFunc() {
  do {
    ...
    ncclCudaLaunchBlocking = val!=NULL && val[0]!=0 && !(val[0]=='0' && val[1]==0);
  } while (0);
  ...
  if (hsaLib == NULL) {
    log_warn("Failed to find ROCm runtime library in %s (RCCL_ROCR_PATH=%s)", ncclCudaPath, ncclCudaPath);
    goto error;
  }
...
//     log_info("Dmabuf feature disabled without NCCL_DMABUF_ENABLE=1");
//     goto error;
//   }
